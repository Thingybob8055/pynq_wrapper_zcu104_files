...
class Console_IO:
    ...
    def __init__(self, axi_dma, recv_buffer_capacity=1004, send_buffer_capacity=1004):
        ...
        self.on_send_callback = lambda msg: None
        self.on_receive_callback = lambda msg: None
    ...
    def receive_transfer(self):
        ...
        if self.recvchannel.transferred > 1:
            new_str = self.recv_buffer[:self.recvchannel.transferred-1].tobytes().decode('utf-8')
            self.recv_str += new_str
            if self.on_receive_callback:
                self.on_receive_callback(new_str)
