...
class ATF_Watchpoints:
    def __init__(self, cms_ctrl):
        ...
        self.watchpoints = {} # key = index, value = dict like:
                        ...
                        #   "attributes_notes" : {"pc":"Program Counter", "A0":"Return value / 1st argument to function"}
                        # }
        ...
        with self.watchpoints_lock:
            for index, watchpoint in self.watchpoints.items():
                if not watchpoint["active"]:
                    continue
                self.cms_ctrl.set_atf_match_rule(int(index), watchpoint["attributes"])
    ...
    def set_watchpoint(self, index, attributes_dict, is_active, masks_dict={}, attributes_notes_dict={}):
        ...
        with self.watchpoints_lock:
            self.watchpoints[index] = {
                ...
                "attributes_notes" : attributes_notes_dict
            }
        if is_active:
            self.cms_ctrl.set_atf_match_rule(int(index), attributes_dict, masks_dict)
        else:
    ...
    def set_watchpoint_active(self, index, is_active):
        ...
        if index not in self.watchpoints:
            print(f"Watchpoint with index {index} does not exist. Can't set active state.")
            return False
        with self.watchpoints_lock:
            self.watchpoints[index]["active"] = is_active
        if is_active:
            self.cms_ctrl.set_atf_match_rule(index, self.watchpoints[index]["attributes"])
        else:
    ...
    def remove_watchpoint(self, index):
        index = int(index)
        with self.watchpoints_lock:
            is_present = index in self.watchpoints
        if not is_present:
            print(f"Watchpoint with index {index} does not exist. Can't remove it.")
            return False
        self.cms_ctrl.reset_atf_match_rule(int(index))
        with self.watchpoints_lock:
