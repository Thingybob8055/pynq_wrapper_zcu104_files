...
class TCP_Server:
    ...
    def __init__(self, host_ip='0.0.0.0', port=9092, verbose=False):
        ...
        self.verbose_queue = queue.Queue()
        ...
        self.verbose_thread = threading.Thread(target=self.verbose_worker, daemon=True)
        
    def start(self):
        self.respond_thread.start()
        self.send_thread.start()
        self.listen_thread.start()
        self.verbose_thread.start()
    ...
    def send_worker(self):
        while True:
            if not self.to_send_queue.empty():
                ...
                if self.verbose:
                    client_name = 'all' if client_id == SEND_TO_ALL else client_id
                    self.verbose_queue.put(
                        # f'Server -> Client string:\n{data}\nPretty print:\n{pprint.pformat(json.loads(data))}\n\n'
                        f'Server -> Client:\n{pprint.pformat(json.loads(data))}\n\n'
                    )
    ...
    def listen_worker(self, listen_socket):
        ...
        if listen_socket.fileno() == -1:
            print('Error: listen_socket.fileno() == -1')
            return
    ...
    def verbose_worker(self):
        ''' The only purpose of this thread is to print debug messages from different threads in synchronized way. '''
        while True:
            if not self.verbose_queue.empty():
                msg = self.verbose_queue.get()
                if self.verbose:
                    print(msg)
            time.sleep(0.01)
